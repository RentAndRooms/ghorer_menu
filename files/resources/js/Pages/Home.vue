<template>
  <CustomerLayout>
      ...
      <div class="relative z-10 w-full max-w-6xl mx-auto m-0 p-5">
        <div class="text-center space-y-8">
          ...
          <h1 class="text-5xl sm:text-6xl md:text-7xl font-bold tracking-tight">
            <span
              class="block text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-red-600 dark:from-orange-500 dark:to-red-500 animate-gradient pb-2">Select
              Your Favourit Food</span>
          </h1>

          <!-- Subheading -->
          <p class="max-w-2xl mx-auto text-xl md:text-2xl text-gray-600 dark:text-gray-300 leading-relaxed">Within Our
            Best Restaurants And Catering Servics</p>
          <!-- Search Field -->
          <div class="relative max-w-xl mx-auto">
            <input type="text" v-model="searchQuery" @input="check" placeholder="type your local area"
            ...
            <ul v-if="searchQuery.length > 0 && showDropDown"
              ...
              <li v-for="(local, index) in filterLocal" :key="index" @click="selectedArea(local)"
    ...
    <TransitionGroup name="list" tag="div" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-6 px-4 md:px-8">
      <div v-for="branch in filteredBranches" :key="branch.id"
        ...
        data-aos="fade-up" style="background-color: rgb(249 115 22);">
        ...
        <div
          ...
          <div class="flex items-center justify-center">
            ...
            <div class="flex flex-col items-center text-center">
              ...
              <!-- Branch Info -->
              <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{{ branch.name }}</h3>
              <address class="mt-1 text-sm dark:text-gray-400 not-italic line-clamp-2" style="font-size: 15px;">{{
                branch.address }}</address>
              <div class="mt-1" style="color: rgb(17 24 39); font-size: 15px;">{{ branch.contact_number }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-5 space-y-5">
          <div>
            <div class="flex flex-wrap items-center justify-center gap-4 text-sm text-gray-600 dark:text-gray-300">
              <!-- Order Now Button -->
              <div>
                <Link :href="route('customer.branch.menu', { branch: branch.id, type: 'collection' })"
            ...
            <div class="flex justify-center mt-2">
              <select v-model="selectedDay[branch.id]" style="text-align: center;"
          ...
          <div class="space-y-3">
            <div
              ...
              <table class="w-full text-sm text-left">
                ...
                <tbody>
                  <tr v-for="(hours, index) in branch.opening_hours.filter(
                    (h) => !selectedDay[branch.id] || h.day === selectedDay[branch.id]
                  )" :key="index" class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <td class="py-3 px-4 text-center">{{ formatTime(hours.breakfast.start) }} - {{
                      formatTime(hours.breakfast.end) }}</td>
                    <td class="py-3 px-4 text-center">{{ formatTime(hours.lunch.start) }} - {{
                      formatTime(hours.lunch.end) }}</td>
                    <td class="py-3 px-4 text-center">{{ formatTime(hours.dinner.start) }} - {{
                      formatTime(hours.dinner.end) }}</td>
                  </tr>
                  <tr v-if="
                  ...
                  ">
                    <td colspan="4" class="py-4 text-center text-gray-500 dark:text-gray-400 italic">Closed</td>
                  </tr>
                </tbody>
              </table>
            </div>
