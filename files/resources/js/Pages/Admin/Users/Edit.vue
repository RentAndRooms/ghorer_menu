<template>
    <AdminLayout>
        <div class="bg-white min-h-screen p-8">
            <h2 class="text-center text-xl text-gray-800">Edit User</h2>
            <form @submit.prevent="handleUpdate" class="max-w-4xl mx-auto text-gray-800">
                <!-- Username -->
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <p v-if="formData.errors.password" class="text-red-500 text-sm mt-1">{{ formData.errors.password }}
                    </p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-6 mb-8">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        <div class="relative">
                            <input type="radio" v-model="formData.role" value="customer" name="role" class="sr-only"
                                required />
                            <div class="w-5 h-5 border-2 border-gray-400 rounded flex items-center justify-center transition-all duration-200"
                                :class="formData.role === 'customer' ? 'bg-green-500 border-green-500' : 'bg-transparent'">
                                <svg v-if="formData.role === 'customer'" class="w-3 h-3 text-white" fill="none"
                        ...
                        <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" stroke-width="1.5"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M15.75 6.75a3 3 0 11-6 0 3 3 0 016 0zM4.5 20.25a8.25 8.25 0 0115 0" />
                        </svg>
                        <span class="text-sm text-gray-800">Consumer</span>
                    </label>

                    <!-- Partner Option -->
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        <div class="relative">
                            <input type="radio" v-model="formData.role" value="branch_admin" name="role"
                                class="sr-only" />
                            <div class="w-5 h-5 border-2 border-gray-400 rounded flex items-center justify-center transition-all duration-200"
                                :class="formData.role === 'branch_admin' ? 'bg-blue-500 border-blue-500' : 'bg-transparent'">
                                <svg v-if="formData.role === 'branch_admin'" class="w-3 h-3 text-white" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                        d="M5 13l4 4L19 7" />
                                </svg>
                        ...
                        <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" stroke-width="1.5"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87M15 11a3 3 0 100-6 3 3 0 000 6zM9 11a3 3 0 100-6 3 3 0 000 6z" />
                        </svg>
                        <span class="text-sm text-gray-800">Partner</span>
                    </label>

                    <!-- Restaurant Dropdown -->
                    <div v-show="formData.role === 'branch_admin'">
                        <select id="rest_id" v-model="formData.rest_id"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 text-gray-800"
                            required>
                            <option value="">Select a Restaurant</option>
                            <option v-for="rest in Restaurant" :key="rest.id" :value="rest.id">{{ rest.name }}</option>
                        </select>
    ...
    data() {
        return {
            formData: useForm({
                ...
                role: this.user.role || '',
                rest_id: this.user.rest_id || '',
            }),
        };
    },
    methods: {
        handleUpdate() {
            this.formData.put(route('admin.user.update', this.user.id), {
                onSuccess: () => {
