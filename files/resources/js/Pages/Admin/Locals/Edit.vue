<template>
    <AdminLayout>
        <div class="bg-white min-h-screen p-8">
            <h2 class="text-center text-xl text-gray-800">Edit Local Area</h2>
    ...
    data() {
        return {
            formData: useForm({
                ...
                city_id: this.local.city_id || '',
                area_id: this.local.area_id || '',
            }),
        };
    },
    methods: {
        handleUpdate() {
            this.formData.put(route('admin.locals.update', this.local.id), {
                onSuccess: () => {
                    console.log('Local area updated successfully');
                },
                onError: (errors) => {
