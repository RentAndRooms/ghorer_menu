      ...
      <nav class="mt-5">
        <ul>
          <li v-for="item in navItems" :key="item.name" class="relative">
            <Link v-if="!item.children && item.canAccess" :href="item.link"
            ...
            <svg v-if="item.icon" class="w-5 h-5 text-gray-600 dark:text-gray-50 mr-2" fill="none" stroke="currentColor"
      ...
      <header class="fixed top-0 left-0 right-0 lg:left-64 shadow-md z-20 flex justify-between items-center p-4">
        ...
        <div class="relative">
          <button @click="toggleUserMenu" class="flex items-center space-x-2 focus:outline-none">
            ...
            <svg class="w-4 h-4 text-gray-600 dark:text-gray-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <div v-if="showUserMenu"
            ...
            <Link href="/profile"
              class="block px-4 py-2 text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-900">Profile
            </Link>
